var url;
var pengguna = $("#pengguna").DataTable({
    responsive: true,
    scrollX: true,
    ajax: { url: readUrl, type: 'POST', cache: false },
    columnDefs: [{ searchable: false, orderable: false, targets: 0 }],
    order: [[0, "asc"]],
    columns: [{ data: null }, { data: "username" }, { data: "nama" }, { data: "action" }]
});

pengguna.on("order.dt search.dt", function() {
    pengguna.column(0, { search: "applied", order: "applied" }).nodes().each(function(el, val) {
        el.innerHTML = val + 1;
    });
}).draw();

function reloadTable() { pengguna.ajax.reload(null, false); }
...
function edit(id) {
    $.ajax({
        url: getPenggunaUrl, type: "post", data: { id: id }, dataType: "json",
