var url;
var kategori_produk = $("#kategori_produk").DataTable({
    responsive: true,
    scrollX: true,
    ajax: { url: readUrl, type: 'POST', cache: false },
    columnDefs: [{ searchable: false, orderable: false, targets: 0 }],
    order: [[0, "asc"]],
    columns: [{ data: null }, { data: "kategori" }, { data: "action" }]
});

kategori_produk.on("order.dt search.dt", function() {
    kategori_produk.column(0, { search: "applied", order: "applied" }).nodes().each(function(el, val) {
        el.innerHTML = val + 1;
    });
}).draw();

function reloadTable() { kategori_produk.ajax.reload(null, false); }
...
function edit(id) {
    $.ajax({
        url: get_kategoriUrl, type: "post", data: { id: id }, dataType: "json",
